// Package wpgithub provides a documentation source adapter that reads
...
// WPGithubSource is a documentation source adapter that reads WordPress
...
var _ source.Source = (*WPGithubSource)(nil)
...
func WithRef(ref string) Option {
	return func(s *WPGithubSource) { s.ref = ref }
}

// New constructs a new WPGithubSource pointing at a local clone of the
...
func (s *WPGithubSource) blobBase() string {
	ref := s.ref
	if ref == "" {
		ref = defaultBlobRef
	}
	return "https://github.com/WordPress/WordPress/blob/" + ref + "/"
}

// ID returns the canonical library ID.
func (s *WPGithubSource) ID() string { return "/wordpress" }
...
func (s *WPGithubSource) DiscoverEntities(ctx context.Context, fetch source.FetchFunc) ([]string, error) {
	idx, err := buildCodebaseIndex(s.repoPath)
	if err != nil {
...
func (s *WPGithubSource) parseClassEntity(body []byte, url string) (*source.Entity, []string, error) {
	analysis := parseFile(body)
...
func (s *WPGithubSource) parseFunctionEntity(body []byte, url string) (*source.Entity, []string, error) {
